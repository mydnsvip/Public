...
echo "from:https://blog.mydns.vip/4792.html"
...
Help() {
	...
	echo "-u | --batchub: Batch ubuntu 20 Binding IPV6"
	...
	echo "Example: ./$(basename $0) -u 240e:d9:c200:101:7bb2::120 240e:d9:c200:101:7bb2::130"
	echo
}
...
ipv6gw="`echo $1|awk -F ':' '{print $1":"$2":"$3":"$4"::1"}'`" #ipv6网关
ipv4="`ip addr |grep "eth0" |grep " inet"|awk '{print $2}'`" #ipv4地址
ipv4gw="`cat /etc/netplan/00-installer-config.yaml | grep "gateway4"`" #ipv4网关
pre=`echo ${1%:*}`
dstart=`printf %d 0X${start}`
dend=`printf %d 0X${end}`
total=$(($dend-$dstart+1))
...
echo "Batch Binding IPV6"
	#echo "IPV6ADDR_SECONDARIES=\"$ipv6string\"" 
	#echo "$ipv6string"
mv /etc/netplan/00-installer-config.yaml /etc/netplan/00-installer-config.yaml_bak
